<hr>
<div class="footer">
    <img src="{{ URL::asset('images/buttons/browser.gif') }}" width="88" height="31" alt="Any Damn Browser">&nbsp;
    <a href="https://dimden.dev/"><img src="https://dimden.dev/services/images/88x31.gif" alt="dimden.dev"></a>&nbsp;
    <a href="https://www.linux.org/"><img src="{{ URL::asset('images/buttons/linuxnow.gif') }}" width="88"
            height="31" alt="Linux NOW!"></a>&nbsp;
    <a href="https://www.vim.org/"><img src="{{ URL::asset('images/buttons/vim.gif') }}" width="88" height="31"
            alt="vim"></a>&nbsp;
    <a href="http://jigsaw.w3.org/css-validator/check/referer"><img
            src="{{ URL::asset('images/buttons/vcss-blue.gif') }}" width="88" height="31"
            alt="Valid CSS!"></a>&nbsp;
    <img src="{{ URL::asset('images/buttons/valid-html401-blue.png') }}" width="88" height="31"
        alt="Valid HTML 4.01!"><br>
    This site is best viewed at 1024x768 with 16-bit color or better<br>
    &copy; floppydisk 2021-{{ date('Y') }}, Site Version {{ config('app.version') }}, <a
        href="https://github.com/floppydisk05/diskfloppy.me">Source</a><br>
</div>
